...
function Sync-Codeowners {
    ...
    $desired = @(
        ...
        '#',
        '# main requires code-owner approval. GitHub Apps cannot be code owners, and that',
        '# is what prevents any bot from merging to production.',
        '',
        # Owner and team both listed. Either satisfies code-owner review, so the gate does
        # not break if the team loses repository access, while adding an approver later
        # becomes a team membership change rather than a commit.
        "* @$OwnerLogin @$Org/pr-approvers",
        '',
        '# The controls that watch the agents are owned explicitly. Without these lines a',
        '# pull request editing the workflows or the governance config is covered only by',
        '# the catch-all, so any owner could wave through a change to the controls.',
        "/.github/ @$OwnerLogin @$Org/pr-approvers",
        "/governance/ @$OwnerLogin @$Org/pr-approvers"
    ) -join "`n"
    ...
    if ($current) {
        # Existence is not the same as validity. A CODEOWNERS naming a team with no access,
